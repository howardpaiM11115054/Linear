...
for id in list:
    ...
    combined_results = []
    # 文件路径配置
    input_file_path = f"./bestdata/MOT17-{id}-FRCNN.txt"  # 输入文件名，确保它在脚本运行的目录下或指定正确的路径
    if not os.path.exists("verygood"):
        os.makedirs("verygood")
    output_file_path = f'./verygood/MOT17-{id}-FRCNN.txt'  # 输出文件名
    # 调用函数执行排序任务
...
# 開啟檔案並解析 "./mot17_val_post/data/MOT17-04-FRCNN.txt" 中的資料
    ...
    for i in range(len(lines) - 1):
        ...
        if abs(float(next_line[0]) - float(current_line[0])) > 1 and abs(float(next_line[0]) - float(current_line[0])) < 10:
            ...
            with open(f"./mot17_val_post/tracks53_0.94/MOT17-{id}-FRCNN.txt", 'r') as gmot_file:
    ...
    # 文件路径
    file1_path = f'./bestdata04/MOT17-{id}-FRCNN.txt'
    ...
    output_file_path = f'./Difference_method03/MOT17-{id}-FRCNN.txt'  # 你可以自定义输出文件的名字
