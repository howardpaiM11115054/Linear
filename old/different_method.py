...
def main():
    ...
    for id in ids:
        input_file_path = f"./merged_final/MOT17-{id}-FRCNN.txt"
        output_file_path = f"./verygood02/MOT17-{id}-FRCNN.txt"
        ...
        # 写入结果到新文件
        if not os.path.exists("bestdata07"):
            os.makedirs("bestdata07")
        with open(f"./bestdata07/MOT17-{id}-FRCNN.txt", 'w') as output_file:
